...
function buscarOpcionMUsuWebVerComentarios(cedula)
{
    creartObjeto();
    xhr.open("GET", "MUsuWebVerComentarios?cedula="+cedula, true);
    xhr.onreadystatechange = verificaDetall;
    xhr.send(null);
}

function verificaDetall() {
    if (xhr.readyState == 4) {
        document.getElementById("ini").innerHTML = "<em>" + xhr.responseText + "</em>";
    }
}
