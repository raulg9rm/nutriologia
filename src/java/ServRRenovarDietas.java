 ...
 * @author Raúl
 */
@WebServlet(urlPatterns = {"/ServRRenovarDietas"})
public class ServRRenovarDietas extends HttpServlet {
    ResultSet cdr=null;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ServRRenovarDietas</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ServRRenovarDietas at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
"        <!--================End Home Banner Area =================-->");
        ...
        try{
            String strComando="select dieta.id_dieta, dieta.nombre, descripcion, duracion, fecha_inicio, fecha_termino, estatus from dieta INNER JOIN compradieta on compradieta.id_dieta=dieta.id_dieta INNER JOIN infodieta on infodieta.id_dieta=dieta.id_dieta where id_paciente='"+idpa+"' and estatus='vigente' group by dieta.id_dieta";
            ...
            out.println("<th>Nombre</th><th>Descripcion</th><th>Duracion (meses)</th><th>Inicio</th><th>Termino</th><th></th>");
            out.println("</tr>");
            while(cdr.next()){
                cont++;
                out.println("<tr>");
                diet=cdr.getString("id_dieta");
                ...
                out.println("<td id='"+cont+"'width=10%>" + cdr.getString("fecha_inicio") + "</td>");
                out.println("<td id='"+cont+"'width=10%>" + cdr.getString("fecha_termino") + "</td>");
                out.println("<td width=10%><button id='el' value='0' class=\"main_btn2\" onclick='renovarPlanDieta2("+diet+");'>Renovar</button></td>");
                out.println("</tr>");
            ...
            cont=0;
                ...
                out.println("<div id=\"detalleMisDietaRenovar\" align=\"center\">\n" +
"                                            \n" +
"                                    </div>");
                out.close();
