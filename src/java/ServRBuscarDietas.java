 ...
 * @author Raúl
 */
@WebServlet(urlPatterns = {"/ServRBuscarDietas"})
public class ServRBuscarDietas extends HttpServlet {
    ResultSet cdr=null;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ServRBuscarDietas</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ServRBuscarDietas at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
"        <!--================End Home Banner Area =================-->");
            String strComando2="SELECT * FROM nutriologo where cedula='"+cedula+"';";
            cdr = sentenciaSQL.executeQuery(strComando2);
            while(cdr.next()){
                usuu=cdr.getString("usuario");
            }
            
             String strComando3="SELECT * FROM usuario where usuario='"+usuu+"';";
            cdr = sentenciaSQL.executeQuery(strComando3);
            while(cdr.next()){
                out.println("<center><h2>Dietas disponibles del nutri&oacutelogo: "+cdr.getString("nombre")+" "+cdr.getString("paterno")+" "+cdr.getString("materno")+"</h2></center>");
            }
            
            String strComando="SELECT * FROM dieta where cedula='"+cedula+"';";
            ...
            out.println("<br>");
            out.println("<center><table width=50% border=2>");
            out.println("<tr>");
            
            out.println("<th>Nombre</th><th>Descripcion</th><th>Duracion (meses)</th><th>Costo</th><th>Cedula de nutriologo</th><th> </th>");
            out.println("</tr>");
            while(cdr.next()){
                cont++;
                out.println("<tr>");
                ...
                out.println("<td id='"+cont+"'width=10%>" + cdr.getString("cedula") + "</td>");
                out.println("<td width=10%><button id='el' class=\"main_btn2\" value='0' onclick='comprarDieta("+cdr.getString("id_dieta")+","+cedula+","+cdr.getString("costo")+");'>Comprar</button></td>");
                out.println("</tr>");
            ...
            cont=0;
                ...
                out.println("<div id=\"detalleDieta\" align=\"center\">\n" +
"                                            \n" +
"                                    </div>");
                out.close();
